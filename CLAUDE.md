...
- **`leetcode-teacher` skill** — Socratic LeetCode/ML implementation teacher with evidence-based learning science. References cover algorithm frameworks, problem patterns, data structures (enriched tree/BST/heap/trie/graph interview cheatsheets), sorting, linked lists (with interview corner cases and complexity tables), arrays, binary search, stacks/queues (with fundamentals sections and Python deque tip), matrix techniques (spiral traversal, rotation, set zeroes), graphs (with 2D matrix DFS/BFS templates and interview tips), brute-force search (backtracking variants, island problems, state-space BFS), dynamic programming (knapsack, grid DP, interval DP, game theory, string matching, egg drop, max subarray, shortest path DP), greedy algorithms (interval scheduling, jump game, gas station, scan line), other common techniques (bit manipulation, math techniques, brain teasers/games, probability/randomness, classic interview problems including trapping rain water, ugly numbers, pancake sorting, perfect rectangle, interval merging with corner cases and utility functions), combinatorics (Legendre's formula, binomial coefficients, Catalan numbers, inclusion-exclusion, Burnside/Polya, stars and bars, generating combinations, bishops on chessboard, bracket sequences, counting labeled graphs), numerical search (bisection, ternary search, Newton's method, simulated annealing), numerical integration (Simpson's rule, trapezoidal rule), and ML implementations.
