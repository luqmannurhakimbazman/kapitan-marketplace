...
- **`leetcode-teacher` skill** — Socratic LeetCode/ML implementation teacher with evidence-based learning science. References cover algorithm frameworks, problem patterns, data structures, sorting, linked lists, arrays, binary search, stacks/queues, graphs, brute-force search (backtracking variants, island problems, state-space BFS), dynamic programming (knapsack, grid DP, interval DP, game theory, string matching, egg drop, max subarray, shortest path DP), greedy algorithms (interval scheduling, jump game, gas station, scan line), other common techniques (bit manipulation, math techniques, brain teasers/games, probability/randomness, classic interview problems including trapping rain water, ugly numbers, pancake sorting, perfect rectangle, interval merging), and ML implementations.
